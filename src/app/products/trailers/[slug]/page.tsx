...
export async function generateStaticParams() {
  return products.trailers.map((trailer) => ({
    slug: trailer.id,
  }));
}

export async function generateMetadata({ params }: { params: { slug: string } }) {
  const trailer = products.trailers.find((p) => p.id === params.slug);
  if (!trailer) {
    return {
  ...
  return {
    title: `${trailer.name} | Thorat Motors`,
    description: trailer.description,
  };
}

export default function TrailerDetailPage({ params }: { params: { slug: string } }) {
  const trailer = products.trailers.find((p) => p.id === params.slug);
      ...
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="mt-16 text-center">
            <Button asChild size="lg" className="bg-accent text-accent-foreground hover:bg-accent/90">
                <Link href="/quote">Request a Quote for this Product</Link>
            </Button>
        </div>
      </div>
    </div>
